...
const Create = ({onHide, addModal, setAddModalFooter, csrf_token, toast, setWorkers,dealers}) => {
    ...
    const {values, handleSubmit, handleChange, setFieldValue, errors, setErrors} = useFormik({
        ...
        onSubmit: values => {
            ...
            fetch(route('super.workers.store'), {
                method: 'POST',
                headers: {
    ...
    useEffect(() => {
        setAddModalFooter(<>
            <Button label="Vazgeç" icon="pi pi-times" size={"small"} link onClick={() => {
                onHide();
            }} loading={loading}/>
            <Button label="Kaydet" icon="pi pi-save" type={"button"} size={"small"} className="p-button-success"
                    loading={loading}
                    onClick={() => {
    ...
    return (<BlockUI blocked={loading} template={<i className="pi pi-spin pi-spinner" style={{fontSize: '3rem'}}></i>}>
        <form className="p-fluid" onSubmit={handleSubmit}>
            ...
            <div className="mb-3 flex justify-start items-center gap-x-2">
                <InputSwitch checked={values.active} inputId={"dealerActive"}
                             onChange={(e) => setFieldValue('active', e.value)}/>
                <label htmlFor="dealerActive" className="font-bold">
            ...
            <div className={"mb-3 grid grid-cols-2 gap-x-3"}>
                ...
                <div>
                    ...
                    <Password id={"password_confirmation"} name={"password_confirmation"}
                              value={values.password_confirmation} onChange={handleChange} feedback={false} toggleMask/>
                </div>
            </div>
