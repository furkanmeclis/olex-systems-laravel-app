...
export default function Update({
    ...
    const dealerId = Number(worker.parent_id);
    const [selectedDealer,setSelectedDealer] = useState(dealers.find(dealer => dealer.id === dealerId));
    ...
    const {values ,handleSubmit, handleChange,setFieldValue,errors,setErrors} = useFormik({
        ...
        validationSchema: Yup.object().shape({
            name: Yup.string().required('Çalışan Adı Zorunludur.'),
            ...
            password: Yup.string()
                ...
                .equals([Yup.ref('password_confirmation')], 'Şifreler Eşleşmiyor.')
        ...
        onSubmit: values => {
            ...
            fetch(route('super.workers.update',worker.id), {
                method: 'POST',
                headers: {
        ...
        <form className="p-fluid" onSubmit={handleSubmit}>
            <div className="grid grid-cols-2 gap-x-3 mb-3">
                ...
                <div className={"col-span-2"}>
                    <label htmlFor="name" className="font-bold">
                        Çalışan Adı <span className={"font-semibold text-red-400"}>*</span>
                    </label>
            ...
            <div className={"mb-3"}>
                <label htmlFor="email" className="font-bold">
                    Çalışan E-Posta Adresi <span className={"font-semibold text-red-400"}>*</span>
                </label>
                <InputText id="email" type={"email"} name={"email"} onChange={handleChange} value={values.email}/>
            </div>
            <div className={"mb-3"}>
                <label htmlFor="phone" className="font-bold">
                    Çalışan Telefon No <span className={"font-semibold text-red-400"}>*</span>
                </label>
            ...
            <div className="mb-3 flex justify-start items-center gap-x-2">
                <InputSwitch checked={values.active} inputId={"workerActive"} onChange={(e) => setFieldValue('active',e.value)} />
                <label htmlFor="workerActive" className="font-bold">
                    Üye Aktifliği ({values.active ? "Aktif" : "Pasif"})
                </label>
            </div>
            <div className="mb-3 flex justify-start items-center gap-x-2">
                <InputSwitch checked={values.reset_password} inputId={"resetPassword"} onChange={(e) => setFieldValue('reset_password',e.value)} />
                <label htmlFor="resetPassword" className="font-bold">
                    Şifre Sıfırlama
                </label>
            </div>
            {values.reset_password && <div className={"mb-3 grid grid-cols-2 gap-x-3"}>
                <div>
                    <label htmlFor="password" className="font-bold">
                        Çalışan Şifre <span className={"font-semibold text-red-400"}>*</span>
                    </label>
                ...
                <div>
                    <label htmlFor="password_confirmation" className="font-bold">
                        Çalışan Şifre Onayla<span className={"font-semibold text-red-400"}>*</span>
                    </label>
            ...
            <div className={"mb-3"}>
                <label htmlFor="phone" className="font-bold">
                    Bayi Seçiniz <span className={"font-semibold text-red-400"}>*</span>
                </label>
                <Dropdown value={selectedDealer} filter emptyFilterMessage={"Bayi Bulunamadı"} filterBy={"name,email,phone"} onChange={(e) => setSelectedDealer(e.value)} options={dealers} optionLabel="name"
                          className="w-full" />
            </div>
