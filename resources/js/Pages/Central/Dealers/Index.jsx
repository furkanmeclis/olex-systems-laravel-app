...
import Create from "@/Pages/Central/Dealers/Create.jsx";
import Update from "@/Pages/Central/Dealers/Update.jsx";
...
export default function Index({auth, dealersAll, csrf_token}) {
                        ...
                        <Button size={"small"}
                                onClick={(event) => {
                                    confirmPopup({
                                        ...
                                        accept: () => {
                                            let url = route('central.dealers.multipleDestroy');
                                            fetch(url, {
                                                method: 'POST',
                                                headers: {
    ...
    const confirmPopupForDealerDelete = (event, itemId, item = "dealer") => {
        const deleteFunction = () => {
            fetch(route('central.dealers.destroy', itemId), {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': csrf_token
                            ...
                            {selectedColumns.includes('actions') && <Column header="İşlemler" body={(user) => {
                                return <div className={"flex justify-center gap-x-2"}>
                                    <Button icon="pi pi-eye" size={"small"} tooltip={"Bayiyi Görüntüle"}
                                        ...
                                        router.visit(route('central.dealers.show', user.id));
                                    }}/>
